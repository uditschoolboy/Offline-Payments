...
public class ScanQrCode extends AppCompatActivity {
    ...
    private void finishThisActivity(String upi_id){
                    ...
                    txtBarcodeValue.post(new Runnable() {
                        ...
                        public void run() {
                            ...
                            for(int i=0;i<barcodes.size();i++) {
                                String tmp = barcodes.valueAt(i).displayValue;
                                Uri uri = Uri.parse(tmp);
                                String upi_id = uri.getQueryParameter("pa");
                                if (upi_id != null) {
                                    isEmail = false;
                                    txtAction.setText("UPI id is Fetched");
                                    //intentData = upi_id;
                                    txtBarcodeValue.setText(upi_id);
                                    System.out.println("Upi id: "+upi_id);
                                    finishThisActivity(upi_id);
                                    return;
                                }
                            }
                        }
